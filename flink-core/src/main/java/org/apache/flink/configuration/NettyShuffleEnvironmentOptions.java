            ...
            ConfigOptions.key(HYBRID_SHUFFLE_NEW_MODE_OPTION_NAME)
                    ...
                    .withDescription(
                            "The option is used to enable the new mode of hybrid shuffle, which has resolved existing issues in the legacy mode. First, the new mode "
                                    ...
                                    + " is configured. The new mode is currently in an experimental phase. The option can be set to false to fallback to the legacy "
                                    + "mode if the new mode is unstable. Once the new mode reaches a stable state, the legacy mode and the option will be removed.");
