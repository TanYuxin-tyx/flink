...
/** The buffer container to store the accumulated records for {@link SortBufferAccumulator}. */
class SortBufferContainer {
    /**
    ...
    public SortBufferContainer(
            LinkedList<BufferBuilder> availableBuffers,
        ...
        checkArgument(availableBuffers.size() > 0, "Empty available buffers.");

        this.freeSegments = new LinkedList<>();
        while (!availableBuffers.isEmpty()) {
            BufferBuilder bufferBuilder = availableBuffers.poll();
            Buffer buffer = bufferBuilder.createBufferConsumerFromBeginning().build();
            freeSegments.add(buffer.getMemorySegment());
